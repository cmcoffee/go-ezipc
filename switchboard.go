...
func (s *session) switchboard(c *connection, req *msg) {		
	...
	if req.Err == "" && req.Tag > 0 {
		...
		if dst, ok := s.localMap[req.Dst]; ok {
			...
			s.setBusy(req)
			if err := s.route(dst(req)); err != nil { s.log.Println(err) }
			s.unsetBusy(req)

			// Setup direct connection to caller, cut out middleman.
			if s.uplink == c && req.Dst != myAddr { 
				if err := s.peerConnect(req.Dst); err != nil {
					s.log.Println(err)
					s.add_route(req.Dst, c)
				}
			}
			return
		}
	...
	if req.Err == "" {
		...
		req.Src = dst
		req.Va1 = nil
		if req.Tag < 0 { req.Tag = req.Tag * -1 }
