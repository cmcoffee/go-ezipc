...
func wrapFunc(fptr interface{}) (newFunc func(*msg)(*msg), err error) {
	...
	if fn.Kind() != reflect.Func { return nil, fmt.Errorf("Only functions may be registered, got %s.", fn.Kind().String()) }
	...
	varCheck := func(input reflect.Type) bool {
		...
		if input.Kind() != reflect.Ptr { return true }
		if input.Elem().Kind() != reflect.Struct { return true }

		var name []rune
		name = []rune(input.Elem().Name())
		if len(name) < 1 { return false }
		if unicode.IsUpper(name[0]) { return true }
		return false
	} 
	
	if !varCheck(fn.In(0)) { return nil, errors.New("Method must use exported (or builtin) argument.")}
	if fn.In(1).Kind() != reflect.Ptr || !varCheck(fn.In(1)) { return nil, errors.New("Second argument or Reply must be ptr to exported (or builtin) value.")}
	if fn.NumOut() != 1 || fn.Out(0).Name() != "error" { return nil, errors.New("Method must return only an error.")}
...
func (cl *Caller) RegisterName(name string, fptr interface{}) (err error) {
	...
	switch reflect.TypeOf(fptr).Kind() {
		...
		if name == "" {  name = strings.TrimPrefix(runtime.FuncForPC(reflect.ValueOf(fptr).Pointer()).Name(), "main.") }
